...
(define (set-variable-value! var val env)
  (define (env-loop env) 
	...
	(if (eq? env the-empth-environment)
...
;; the primitive procedure `read` waits for input from the user, and returns the next
;; complete expression that is typed. For example, if the user types (+ 23 x) , read
;; returns a three-element list containing the symbol + , the number 23, and the symbol x 
;; If the user types ’x , read returns a two-element list containing the symbol quote and the symbol x .
...
(define (analyze-variable exp)
  (lambda (env) (lookup-variable-val exp env)))
